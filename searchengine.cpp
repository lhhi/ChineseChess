...
void SearchEngine::addpiece(ChessBoard& w,int id,int x,int y,int turn){
    if(id<=7){
        w.redvalue+=positionvalue[id][x][y]+(!turn)*3;
    }else{
        w.blackvalue+=positionvalue[id][x][y]+turn*3;
    }
}
...
int SearchEngine::evaluate(ChessBoard &w,int turn){
    ...
    for(int i = 0; i < 10; i++){
        for(int j = 0; j < 9; j++){
            ...
            if(id<=7) redvalue+=positionvalue[id][i][j]+(!turn)*3;
            else blackvalue+=positionvalue[id][i][j]+turn*3;
        }
    }
    //w.redvalue=redvalue;
...
moveway SearchEngine::aimode(ChessBoard& w,int deep){
    ...
    for(moveway move:moves){
        Chesspiece* p=Makemove(w,move,0);
        int score=search(w,deep-1,INT_MIN,INT_MAX,false);
        Unmakemove(w,move,p,0);
        if(score>bestscore){
            bestscore=score;
            bestmove=move;
        }
